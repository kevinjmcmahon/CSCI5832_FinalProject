# -*- coding: utf-8 -*-
"""CSI5832_hptune_st1_3.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1oew45_PyO_sbnh5l2hiRu8Z-MuFRl1AE
...
data_dir = base_dir + 'data/subtask1/'
...
for lang in languages:
    ...
    # prepend language token to text column
    train_dfs[lang]["text"] = "[" + lang + "] " + train_dfs[lang]["text"].astype(str)

    # Load dev
    dev_dfs[lang] = pd.read_csv(data_dir + f"dev/{lang}.csv")
    dev_dfs[lang]["language"] = lang
    # prepend language token to text column
    dev_dfs[lang]["text"] = "[" + lang + "] " + dev_dfs[lang]["text"].astype(str)
...
"""### Best hyper-parameters from tuning"""

best_params = {
    'learning_rate': 6.4474876947936455e-06,
    'num_train_epochs': 5,
    'per_device_train_batch_size': 16,
    'weight_decay': 0.05623782158161189,
    'warmup_ratio': 0.04591670111852694,
    'dropout': 0.14912291401980418
}

"""### Final re-training using all of the labelled training data"""

# Uses the full training file (not just the 80% split)
...
"""### Re-training w/ opt parameters & saving the model"""
...
# 2. Build model with best dropout
final_model = build_model(dropout=best_params["dropout"])

# 3. Choose what data to train on:
#    Option A: all training data
final_train_dataset = train_full_dataset
#    Option B (if you prefer): only the 80% split
# final_train_dataset = train_dataset

# 4. Where to save the final model
output_dir = base_dir + "models/subtask1_xlmr_langLabels/"
...
training_args = TrainingArguments(
    ...
    # For the final run you can skip eval or keep it just for logging
    save_strategy="epoch",
    save_total_limit=1,
    load_best_model_at_end=False,   # final run, no need for internal model selection
...
final_trainer = Trainer(
    ...
    eval_dataset=None,  # or val_dataset if you want metrics during training
    tokenizer=tokenizer,
    data_collator=data_collator,
    compute_metrics=None,  # or compute_metrics_multilabel if you kept eval
)

print("Starting final training with best hyper-parameters...")
final_trainer.train()
